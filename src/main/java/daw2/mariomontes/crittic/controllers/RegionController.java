...
import org.iesalixar.daw2.mariomontes.dwese_ticket_logger_api.services.FileStorageService;
import org.iesalixar.daw2.mariomontes.dwese_ticket_logger_api.services.RegionService;
...
public class RegionController {
    ...
    public ResponseEntity<Page<RegionDTO>> getAllRegions(
        ...
        logger.info("Solicitando la todas las regiones con paginación: página {},  tamaño {}",
    ...
    public ResponseEntity<?> createRegion(@Valid @ModelAttribute RegionCreateDTO createDTO) {
        try {
            RegionDTO regionDTO = regionService.createRegion(createDTO);
     ...
     * @param updateDTO DTO con los datos para actualizar la región.
    ...
    public ResponseEntity<?> updateRegion(
            ...
            @Valid @ModelAttribute RegionCreateDTO updateDTO,
            Locale locale) {

        try {
            RegionDTO updatedRegion = regionService.updateRegion(id, updateDTO);
    ...
    public ResponseEntity<?> deleteRegion(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.ok("Región eliminada con exito.");
        } catch (IllegalArgumentException e) {
            logger.warn("Error al eliminar la región con ID {}: {}", id, e.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
        } catch (Exception e) {
            logger.error("Error inseperado al eliminar la región con ID {}: {}", id, e.getMessage());
