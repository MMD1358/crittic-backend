...
import org.iesalixar.daw2.mariomontes.dwese_ticket_logger_api.services.CustomUserDetailsService;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        final String username;  // Variable para almacenar el nombre de usuario extraido del token
        // 2. Verificar si el encabezado Authorization está presente y tiene un token válido
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            // Si el encabezado no está presente o no comienza con "Bearer ", para la solicitud al siguiente filtro
            filterChain.doFilter(request, response);
            return; // Termina la ejecución de este filtro
        }
        // 3. Extraer el token JWT del encabezado (sin el prefijo "Bearer ")
        jwt = authHeader.substring(7); // Elimina los primeros 7 carácteres ("Bearer ")
